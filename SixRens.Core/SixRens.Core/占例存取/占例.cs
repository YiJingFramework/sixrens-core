...
namespace SixRens.Core.占例存取
{
    public sealed class 占例
    {
        ...
        {
            var 年月日时 = this.壬式.起课参数.年月日时;
            _ = builder
                .AppendLine(
                $"{年月日时.年干:C}{年月日时.年支:C}年{空格}" +
                $"{年月日时.月干:C}{年月日时.月支:C}月{空格}" +
                $"{年月日时.日干:C}{年月日时.日支:C}日{空格}" +
                $"{年月日时.时干:C}{年月日时.时支:C}时{空格}" +
                $"{年月日时.月将:C}将")
                .AppendLine(
                $"{年月日时.旬所在.旬首干:C}{年月日时.旬所在.旬首支:C}旬{空格}" +
                $"{年月日时.旬所在.空亡一:C}{年月日时.旬所在.空亡二:C}空");
            if(西历时间.HasValue)
        ...
        private void 追加年命(StringBuilder builder)
        {
            ...
            bool 有内容 = false;
            if (this.壬式.起课参数.课主年命 is not null)
            {
                有内容 = true;
                _ = builder.AppendLine($"课主：{转字符串(this.壬式.起课参数.课主年命)}");
            }
            if (this.壬式.起课参数.对象年命.Count != 0)
            {
                有内容 = true;
                var 年命字符串表 = this.壬式.起课参数.对象年命.Select(年命 => 转字符串(年命));
        ...
        private void 追加三传(StringBuilder builder)
        {
            ...
            {
                var 旬 = this.壬式.起课参数.年月日时.旬所在;
                var 临地 = this.壬式.天地盘.取临地(支);
                var 落空 = 旬.获取对应天干(临地).HasValue ? $"{空格}{空格}" : "落空";
                var 六亲 = this.壬式.起课参数.年月日时.日干.判断六亲(支);
                var 遁干 = 旬.获取对应天干(支)?.ToString("C") ?? 空格;
                var 天将 = this.壬式.天将盘.取乘将(支).简称();
        ...
        private void 追加四课(StringBuilder builder)
        {
            _ = builder.AppendLine(
                $"{空格}{空格}{空格}{空格}" +
                $"{this.壬式.天将盘.取乘将(this.壬式.四课.辰阴).简称()}" +
                $"{this.壬式.天将盘.取乘将(this.壬式.四课.辰阳).简称()}" +
                $"{this.壬式.天将盘.取乘将(this.壬式.四课.日阴).简称()}" +
                $"{this.壬式.天将盘.取乘将(this.壬式.四课.日阳).简称()}")
                .AppendLine(
                $"{空格}{空格}{空格}{空格}" +
        ...
        private void 追加天盘(StringBuilder builder)
        {
            var 天盘 = Enumerable.Range(1, 12)
                .ToDictionary(i => i, i => this.壬式.天地盘.取乘神(new(i)));

            _ = builder
                .AppendLine(
                $"{空格}{空格}{空格}{空格}" +
                $"{this.壬式.天将盘.取乘将(天盘[6]).简称()}{this.壬式.天将盘.取乘将(天盘[7]).简称()}" +
                $"{this.壬式.天将盘.取乘将(天盘[8]).简称()}{this.壬式.天将盘.取乘将(天盘[9]).简称()}")
                .AppendLine(
                $"{空格}{空格}{空格}{空格}" +
                $"{天盘[6]:C}{天盘[7]:C}" +
                $"{天盘[8]:C}{天盘[9]:C}")
                .AppendLine(
                $"{空格}{空格}{空格}" +
                $"{this.壬式.天将盘.取乘将(天盘[5]).简称()}" +
                $"{天盘[5]:C}" +
                $"{空格}{空格}" +
                $"{天盘[10]:C}" +
                $"{this.壬式.天将盘.取乘将(天盘[10]).简称()}")
                .AppendLine(
                $"{空格}{空格}{空格}" +
                $"{this.壬式.天将盘.取乘将(天盘[4]).简称()}" +
                $"{天盘[4]:C}" +
                $"{空格}{空格}" +
                $"{天盘[11]:C}" +
                $"{this.壬式.天将盘.取乘将(天盘[11]).简称()}")
                .AppendLine(
                $"{空格}{空格}{空格}{空格}" +
                $"{天盘[3]:C}{天盘[2]:C}" +
                $"{天盘[1]:C}{天盘[12]:C}")
                .AppendLine(
                $"{空格}{空格}{空格}{空格}" +
                $"{this.壬式.天将盘.取乘将(天盘[3]).简称()}{this.壬式.天将盘.取乘将(天盘[2]).简称()}" +
                $"{this.壬式.天将盘.取乘将(天盘[1]).简称()}{this.壬式.天将盘.取乘将(天盘[12]).简称()}")
                .AppendLine();
        }
