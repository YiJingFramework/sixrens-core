...
namespace 测试用交互
{
    public partial class 起课界面 : Form
    {
        ...
        public 起课界面(插件包管理器 插件包管理器, 预设管理器 预设管理器)
        {
            ...
            this.活时起课绑定 = new();
            foreach (var d in this.地支表.Keys.Select(c => c.ToString()).Prepend("正时"))
                this.活时起课绑定.Add(d);
            this.comboBox1.DataSource = this.活时起课绑定;
            this.comboBox1.SelectedIndex = 0;

            this.月将绑定 = new();
            foreach (var d in this.地支表.Keys.Select(c => c.ToString()).Prepend("自动"))
                this.月将绑定.Add(d);
            this.comboBox2.DataSource = this.月将绑定;
            this.comboBox2.SelectedIndex = 0;

            this.昼夜绑定 = new();
            foreach (var d in new string[] { "自动", "昼占", "夜占" })
                this.昼夜绑定.Add(d);
            this.comboBox4.DataSource = this.昼夜绑定;
            ...
            this.button1.Click += (_, _) => this.设置时间(this.textBox1.Text);
        ...
        {
            this.活时起课绑定[0] = $"正时（{信息.时支:C}）";
            this.月将绑定[0] = $"自动（{(可提供月将 ? 信息.月将.ToString("C") : "无")}）";
            this.昼夜绑定[0] = $"自动（{(信息.昼占 ? "昼占" : "夜占")}）";
        }

        private void button3_Click(object sender, EventArgs e)
        {
            var 课主 = this.本命绑定.Any(i => i.课主 is 本命项目.课主项.课主) ?
                本命项目.课主项.非课主 : 本命项目.课主项.课主;
            this.本命绑定.Add(new(本命项目.性别项.男, 本命项目.本命项.子, 课主));
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (this.comboBox1.SelectedIndex is not 0)
                年月日时 = 年月日时.修改时间(this.地支表[((string)this.comboBox1.SelectedItem)[0]]);
            if (!年月日时.检验八字(false))
            ...
            if (this.comboBox2.SelectedIndex is not 0)
                年月日时 = 年月日时.修改信息(this.地支表[((string)this.comboBox2.SelectedItem)[0]]);
        ...
        private void 设置时间(string s)
        {
            ...
            else if (s.StartsWith("n"))
            {
                ...
                if (this.天干表.TryGetValue(sp[0][0], out var 年干) &&
                    this.天干表.TryGetValue(sp[1][0], out var 月干) &&
                    this.天干表.TryGetValue(sp[2][0], out var 日干) &&
                    this.天干表.TryGetValue(sp[3][0], out var 时干) &&
                    this.地支表.TryGetValue(sp[0][1], out var 年支) &&
                    this.地支表.TryGetValue(sp[1][1], out var 月支) &&
                    this.地支表.TryGetValue(sp[2][1], out var 日支) &&
                    this.地支表.TryGetValue(sp[3][1], out var 时支))
                {
